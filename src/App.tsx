    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-[1] flex items-center justify-center">
        <div className="flex items-center gap-12 w-full max-w-4xl">
          ...
          {/* 班组介绍 */}
          <div className="flex-[2] bg-[#111] border-l-4 border-[#FF6B00] p-4">
...
const ImprovementTimeline = () => {
  ...
  const getYPosition = (index: number, total: number) => {
    const amplitude = 120; // 波浪幅度
    const frequency = 2; // 波浪频率
    const baseY = 200; // 基准Y位置
    const phase = (index / (total - 1 || 1)) * Math.PI * frequency;
    ...
    <SlideWrapper title="历年改善成果" icon={Award}>
      <div className="h-full w-full overflow-hidden relative">
        ...
        <svg className="absolute inset-0 w-full h-full" style={{ minHeight: '600px' }}>
          ...
          {/* 曲线路径 */}
          <motion.path
            d={`M 40 ${getYPosition(0, allImprovements.length)} ${allImprovements.map((_, i) => {
              const x = 40 + (i * ((1920 - 80) / (allImprovements.length - 1 || 1)));
              const y = getYPosition(i, allImprovements.length);
              return `L ${x} ${y}`;
            }).join(' ')}`}
            fill="none"
        ...
        {/* 改善项节点 */}
        <div className="relative h-full flex items-center" style={{ padding: '0 20px' }}>
          <div className="w-full relative" style={{ height: '500px' }}>
            {allImprovements.map((item, index) => {
              const xPercent = (index / (allImprovements.length - 1 || 1)) * 100;
              const yPos = getYPosition(index, allImprovements.length);
              const isTop = yPos < 200;
              ...
              return (
                ...
                >
                  {/* 节点圆点 */}
                  <div 
                    className="w-4 h-4 rounded-full border-4 border-[#0A0A0B] animate-pulse"
                    style={{ backgroundColor: getAwardColor(item.award) }}
                  />
                  
                  {/* 内容卡片 */}
                  <div 
                    className="absolute w-48 bg-[#111]/95 border rounded-lg p-3 backdrop-blur-sm"
                    style={{ 
                      borderColor: getAwardColor(item.award),
        ...
        <div className="absolute bottom-4 left-8 flex gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#888]">一等奖</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-[#00F5FF]" />
            <span className="text-xs text-[#888]">二等奖</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-[#888]" />
            <span className="text-xs text-[#888]">三等奖</span>
          </div>
        </div>
        ...
        <div className="absolute bottom-4 right-8 flex gap-8">
          <div className="text-center">
            <div className="text-3xl font-black text-[#00F5FF]">{allImprovements.length}</div>
